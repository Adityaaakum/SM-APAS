...
public class WorkItems_Consolidation_Test extends TestBase implements testdata, modules, users {
	...
	public void WorkItem_Consolidation(String loginUser) throws Exception {
		...
		String Workitem = "WI-00000691";
		  ...
		  String workItemCreationData = System.getProperty("user.dir") +
		  testdata.MANUAL_WORK_ITEMS; 
		  ...
		  softAssert.assertEquals(objWorkItemHomePage.ErrorMessage.getText(),"Please select two or more Work Items from the same Work Pool and having the same APN/Account # to consolidate.","SMAB-T2260,SMAB-T2260 Validation on workitem with diff APN and Work pool can not consolidate "); objWorkItemHomePage.Click(objWorkItemHomePage.CloseErrorMsg); 
		  ...
		  objWorkItemHomePage.
		  clickElementOnVisiblity("//label[text()='Select Primary']/following-sibling::div//span[contains(@title,'"
		  + PrimaryWorkitem + "')]");///
		  objWorkItemHomePage.Click(objPage.getButtonWithText("Save"));
		  // steps 11: verify primary work item should be visible in in progress tab after consolidation
		  Thread.sleep(2000); HashMap<String, ArrayList<String>> PrimaryWorkItems =
		  ...
		  PrimaryWorkitem)
		  ,"SMAB-T2261: Validation that Primary WorkItem should be visible In In Progress Tab On Home page after consolidate "
		  ); softAssert.assertTrue(!(PrimaryWorkItems.get("Work Item Number").contains(
		  secondaryWorkitem))
		  ,"SMAB-T2261: Validation that Secondary WorkItem should not be visible In In Progress Tab On Home page after consolidate "
		  ...
		  Thread.sleep(2000); WebElement element = driver.findElement(By.
		  xpath("//div[contains(@class,'windowViewMode-normal') or contains(@class,'windowViewMode-maximized')]//span[text()='"
		  +secondaryWorkitem + "']")); objWorkItemHomePage.scrollToElement(element);
		  objWorkItemHomePage.Click(element);
		  ...
		  softAssert.assertEquals(objWorkItemHomePage.ErrormsgOnWI.getText(),"Status: You cannot change status from In Progress to Completed","SMAB-T2287 : Error validation You cannot change status from In Progress to Completed On WI page" ); 
