...
public class EFileImportToolLoad_Test extends TestBase {
	...
	public EFileImportToolLoad_Test(String fileType, String source,String fileImport, String fileName) {
	 ...
	 **/
	@Test(description = "Below test scenario is to upload different file types through Efile Intake for BP and BPP Trends", dataProvider = "loginBPPBusinessAdmin", dataProviderClass = DataProviders.class, groups = {
			"LoadTest" })
	public void Upload_EFileImportTool(String loginUser) throws Exception {
		 ...
		 */
		   ...
		   //Step0: Creating temporary file with random building permit number
		   String buildingPermitNumber = "BD-2020-" + objUtil.getCurrentDate("hhmmss");
		   String newFileName = fileName + objUtil.getCurrentDate("dd-hhmmss");
		  	   
		   String srcFile = System.getProperty("user.dir") + testdata.LOAD_Test + "\\" + fileName +".xlsx";   
		   
		   String tmpFile = System.getProperty("user.dir") + CONFIG.get("temporaryFolderPath") + newFileName + ".xlsx";
		
		  
		 //FileUtils.replaceString(buildingPermitFile,"<PERMITNO>",buildingPermitNumber,temporaryFile);
		   
		   ExcelUtils exl = new ExcelUtils();
		   exl.setCellValueAndCopy(srcFile, buildingPermitNumber, tmpFile);
		...
		objEfileImportPage.uploadFileOnEfileIntake(fileType, source, fileImport, tmpFile);
