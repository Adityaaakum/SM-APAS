...
public class BppTrend_ConstructionMobileEquipCompostieTable_CalculateAndReCalculate extends TestBase {
	 ...
	 */
	@Test(description = "SMAB-T239,SMAB-T240,SMAB-T241,SMAB-T269,SMAB-T190,SMAB-T194,SMAB-T198,SMAB-T442: Performing validation on CONSTRUCTION MOBILE EQUIP COMPOSITE FACTORS before and after calculation", groups = {"smoke,regression"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 0, enabled = true)
	public void verifyBppTrend_AConstructionMobileEquipCompostieTable_CalculateAndCompare(String loginUser) throws Exception {					
		...
		//Step10: Validation to check whether calculation is successful and table data appears for given table
		boolean isTableVisible = objBppTrnPg.isTableDataVisible(tableName);
		softAssert.assertTrue(isTableVisible, "SMAB-T239: User successfully triggered calculation for '"+ tableName +" 'table");
		...
		objSoftAssert.assertEquals(statusInBppTrendSetup, "Calculated", "SMAB-T241: Table status on Bpp Trend Setup page post calculation");
	 ...
	 */
	@Test(description = "SMAB-T195,SMAB-T196,SMAB-T442: Performing validation on CONSTRUCTION MOBILE EQUIP COMPOSITE FACTORS before and after calculation", groups = {"smoke,regression"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 1, enabled = true)
	public void verifyBppTrend_ConstructionMobileEquipCompostieTable_ReCalculateAndCompare(String loginUser) throws Exception {					
		...
		try {
			...
			updatedSettingValues.put("Construction", "50");
			...
			tableName = "Construction Mobile Equipment Composite Factors";
	 ...
	 */
	@Test(description = "SMAB-T192,SMAB-T193: Calculation of CONSTRUCTION MOBILE EQUIP COMPOSITE FACTORS with missing calculation variables", groups = {"smoke,regression"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 2, enabled = true)
	public void verifyBppTrend_ConstructionMobileEquipCompostieTable_CalculateWithCalculationVariablesMissing(String loginUser) throws Exception {					
		try {
			...
			softAssert.assertContains(actErrorMessage, "Index factors not found for Construction", "SMAB-T192: Error message encountered when triggered calculation with missing calculation variables(Max Index Factor and Min Good Factor)");
			...
			softAssert.assertContains(actErrorMessage, "Index factors not found for Construction", "SMAB-T193: Error message encountered when triggered calculation with missing input tables(BPP Property Index Factors, BPP Property Good Factors)");
