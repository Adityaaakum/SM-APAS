...
public class BppTrendBppTrendSettingTest  extends TestBase {
	...
	@Test(description = "SMAB-T190: Create bpp trend setup entry", dataProvider = "loginSystemAdmin", dataProviderClass = DataProviders.class, priority = 0, enabled = false)
	public void verifyBppTrendCreateTrendSetupEntry(String loginUser) throws Exception {		
		...
		boolean isBppSettingCreated = objBuildPermit.checkNewlyCreatedEntryOnDetailsPage(bppTrendSetupName);
		softAssert.assertTrue(isBppSettingCreated, "SMAB-T418: BPP trend setup created successfully and reflecting on details page");
	}
	
	@Test(description = "SMAB-T190: Create a new bpp trend setting", dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 1, enabled = false)
	public void verifyBppTrendCreateTrendSetting(String loginUser) throws Exception {		
		//Step1: Login to the APAS application using the given user
		ExtentTestManager.getTest().log(LogStatus.INFO, "Executing the tests case with user: " + loginUser);
		objApasGenericFunctions.login(loginUser);
...
//		softAssert.assertTrue(isErrorMsgDislayed, "SMAB-T134: SMAB-T134: Error message displayed: '" + actualErrorMsgOnIncorrectFactorValue + "' for  factor value greater than minimum range.");
//		
...
//		List<String> multipleFactorCorrectVauesList = Arrays.asList(CONFIG.getProperty("CorrectFactorVaues").split(","));
...
//				
//		//Step13: Click ViewAll link to navigate to bpp settings grid and create a new bpp setting entry
...
//		//objBppTrnPg.clickAction(objBppTrnPg.waitForElementToBeClickable(objBuildPermit.newButton));
//		//objBppTrnPg.enterFactorValue(factorValue);
//		//objBppTrnPg.Click(objBuildPermit.saveButton);
//		
//		//Step14: Click ViewAll link to navigate to bpp settings grid and edit existing bpp setting entry
//		objBppTrnPg.clickAction(objBppTrnPg.waitForElementToBeClickable(objBppTrnPg.viewAllBppSettings));
//		Thread.sleep(5000);
//		
//		//Step15: Retrieving the equipment index factor value before editing
...
//		
//		//Step18: Navigating to Bpp Composite Factor Setting section and retrieve existing count of bpp composite factor setting
...
//		objBppTrnPg.Click(objBppTrnPg.locateElement(xpathBppSetupNameInBreadcrumb, 20));
		...
		//Step19: Creating a new BPP Trend Commercial Setting Entry
		...
		//Step20: Creating a new BPP Trend Industrial Setting Entry
		objBppTrnPg.clickAction(objBppTrnPg.waitForElementToBeClickable(objBppTrnPg.dropDownIconBppCompFactorSetting));
		Thread.sleep(2000);
		...
		//Step21: Creating a new BPP Trend Construction Setting Entry
		objBppTrnPg.clickAction(objBppTrnPg.waitForElementToBeClickable(objBppTrnPg.dropDownIconBppCompFactorSetting));
		Thread.sleep(2000);
		objBppTrnPg.clickAction(objBppTrnPg.waitForElementToBeClickable(objBppTrnPg.newBppTrendSettingLink));
		...
		//Step22: Creating a new BPP Trend Agricultural Setting Entry
		objBppTrnPg.clickAction(objBppTrnPg.waitForElementToBeClickable(objBppTrnPg.dropDownIconBppCompFactorSetting));
		Thread.sleep(2000);
		objBppTrnPg.clickAction(objBppTrnPg.waitForElementToBeClickable(objBppTrnPg.newBppTrendSettingLink));
	...
	@Test(description = "SMAB-T229: Check for availability of input factor tables on bpp trend roll year screen", dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 2, enabled = false)
