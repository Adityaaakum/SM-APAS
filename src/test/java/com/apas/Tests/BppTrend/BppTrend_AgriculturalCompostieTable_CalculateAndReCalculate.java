...
public class BppTrend_AgriculturalCompostieTable_CalculateAndReCalculate extends TestBase {
	...
	public void verify_BppTrend_AgriculturalCompositeFactors_CalculateAndCompare(String loginUser) throws Exception {					
		...
		objSoftAssert.assertEquals(statusInBppTrendSetup, "Calculated", "SMAB-T269: Table status on Bpp Trend Setup page post calculation");
	 ...
	 */
	@Test(description = "SMAB-T192,SMAB-T193: Calculation of AGRICULTURAL COMPOSITE FACTORS with missing calculation variables", groups = {"smoke,regression,BppTrend"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 2, enabled = true)
	public void verify_BppTrend_AgriculturalCompositeFactors_CalculateWithCalculationVariablesMissing(String loginUser) throws Exception {					
		try {
			...
			softAssert.assertContains(actErrorMessage, "Bpp Composite Factor setting not found for Agricultural", "SMAB-T192: Error message encountered when triggered calculation with missing calculation variables(Max Index Factor and Min Good Factor)");
