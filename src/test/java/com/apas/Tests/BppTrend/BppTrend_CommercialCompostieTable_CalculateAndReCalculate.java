...
public class BppTrend_CommercialCompostieTable_CalculateAndReCalculate extends TestBase {
	 ...
	 */
	@Test(description = "SMAB-T162,SMAB-T165,SMAB-T190,SMAB-T194,SMAB-T198,SMAB-T442: Performing validation on COMMERCIAL COMPOSITE FACTORS before and after calculation", groups = {"smoke,regression"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 0, enabled = true)
	public void verifyBppTrend_CommercialCompositeFactors_CalculateAndCompare(String loginUser) throws Exception {					
				...
				if (acquiredYearDataFromUI.size() == acquiredYearDataFromExcel.size()) {
					...
					for (int j = 0; j < acquiredYearDataFromExcel.size(); j++) {
						if (!(acquiredYearDataFromExcel.get(j).equals(acquiredYearDataFromUI.get(j)))) {
							...
							softAssert.assertTrue(false, "SMAB-T165: Data for '"+ tableName +" 'for year acquired '" + currentKey + "' and column named '"+ columnNames.get(j) + "' matched. Excel data: "+ acquiredYearDataFromExcel.get(j) + " || UI Data: " + acquiredYearDataFromUI.get(j), true);								
							softAssert.assertTrue(false, "SMAB-T162: Data for '"+ tableName +" 'for year acquired '" + currentKey + "' and column named '"+ columnNames.get(j) + "' matched. Excel data: "+ acquiredYearDataFromExcel.get(j) + " || UI Data: " + acquiredYearDataFromUI.get(j), true);
						}
					}
					if(isDataMatched) {
					...
					softAssert.assertTrue(false, "SMAB-T165: Data for '"+ tableName +" 'for year acquired '"+ currentKey +"' matched.", true);
					softAssert.assertTrue(false, "SMAB-T162: Data for '"+ tableName +" 'for year acquired '"+ currentKey +"' matched.", true);
				}
			} else {
				System.setProperty("isElementHighlightedDueToFailre", "true");
	 ...
	 * 10. Validating the data of UI table against the Trend Calculator excel file:: Test Case/JIRA ID: SMAB-T162, SMAB-T165
	 ...
	 * 
	 * JIRA IDs VALIDATED: SMAB-T162, SMAB-T165, SMAB-T195, SMAB-T196,SMAB-T442
	 */
	@Test(description = "SMAB-T162,SMAB-T165,SMAB-T195,SMAB-T196,SMAB-T442: Performing validation on COMMERCIAL COMPOSITE FACTORS before and after calculation", groups = {"smoke,regression"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 1, enabled = true)
	public void verifyBppTrend_CommercialCompositeFactors_ReCalculateAndCompare(String loginUser) throws Exception {
		...
		try {
			...
			updatedSettingValues.put("Commercial", "50");
			...
			tableName = "Commercial Composite Factors";
	...
	/**
	 * DESCRIPTION: Performing Following Validations <COMMERCIAL COMPOSITE FACTORS>::
	 * 1. Validating error message for missing BPP Settings (Maximum Equip. Index Value):: TestCase/JIRA Id: SMAB-T163, SMAB-T192
	 ...
	 * 
	 * JIRA IDs VALIDATED: SMAB-T163, SMAB-T192, SMAB-T193
	 */
	@Test(description = "SMAB-T163,SMAB-T192,SMAB-T193: Calculation of COMMERCIAL COMPOSITE FACTORS with missing calculation variables", groups = {"smoke,regression"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 2, enabled = true)
	public void verifyBppTrend_CommercialCompositeFactors_CalculateWithCalculationVariablesMissing(String loginUser) throws Exception {					
		try {
			...
			softAssert.assertContains(actErrorMessage, "Bpp Composite Factor setting not found for Commercial", "SMAB-T192: Error message encountered when triggered calculation with missing calculation variables(Max Index Factor and Min Good Factor)");
