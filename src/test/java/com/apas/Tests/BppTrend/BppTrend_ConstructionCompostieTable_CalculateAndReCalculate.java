...
public class BppTrend_ConstructionCompostieTable_CalculateAndReCalculate extends TestBase {
	 ...
	 */
	@Test(description = "SMAB-T253,SMAB-T254,SMAB-T255,SMAB-T190,SMAB-T194,SMAB-T198,SMAB-T442: Performing validation on CONSTRUCTION COMPOSITE FACTORS before and after calculation", groups = {"smoke,regression"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 0, enabled = true)
	public void verifyBppTrend_ConstructionCompostieTable_CalculateAndCompare(String loginUser) throws Exception {					
		...
		softAssert.assertTrue(isTableVisible, "SMAB-T253: User successfully triggered calculation for '"+ tableName +" 'table");
				...
				if (acquiredYearDataFromUI.size() == acquiredYearDataFromExcel.size()) {
					...
					for (int j = 0; j < acquiredYearDataFromExcel.size(); j++) {
						if (!(acquiredYearDataFromExcel.get(j).equals(acquiredYearDataFromUI.get(j)))) {
							...
							softAssert.assertTrue(false, "SMAB-T254: Data for '"+ tableName +" 'for year acquired '" + currentKey + "' and column named '"+ columnNames.get(j) + "' matched. Excel data: "+ acquiredYearDataFromExcel.get(j) + " || UI Data: " + acquiredYearDataFromUI.get(j), true);
						}
					}
					if(isDataMatched) {
						softAssert.assertTrue(true, "SMAB-T254: UI Data for '"+ tableName +" 'for year acquired '"+ currentKey +"' matched with Trend Calculator data.", true);
					}
				} else {
					System.setProperty("isElementHighlightedDueToFailre", "true");
					softAssert.assertTrue(false, "SMAB-T254: Data for '"+ tableName +" 'for year acquired '"+ currentKey +"' matched.", true);
				}
			} else {
				System.setProperty("isElementHighlightedDueToFailre", "true");
				softAssert.assertTrue(false, "SMAB-T254: Data for '"+ tableName +" 'for year acquired '"+ currentKey +"' is present in UI table.", true);
			}
		}
	 ...
	 */
	@Test(description = "SMAB-T195,SMAB-T196,SMAB-T442: Performing validation on CONSTRUCTION COMPOSITE FACTORS before and after calculation", groups = {"smoke,regression"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class, priority = 1, enabled = true)
	public void verifyBppTrend_ConstructionCompostieTable_ReCalculateAndCompare(String loginUser) throws Exception {					
		...
		try {
			...
			tableName = "Construction Composite Factors";
