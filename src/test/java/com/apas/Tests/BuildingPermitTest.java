...
public class BuildingPermitTest extends TestBase {
	...
	public void transactionRecordVerificationBuildingPermitTXT(String loginUser) throws Exception {
		...
		salesforceStandardFunctions.searchApps(apps.EFILE_INTAKE);
		...
		salesforceStandardFunctions.searchApps(apps.EFILE_IMPORT_TRANSACTIONS);
	...
	@Test(description = "SMAB-T361:SMAB-T358: Reverting the error records in building permit import", dataProvider = "loginUsers", priority = 0, alwaysRun = true, enabled = false)
	...
	@Test(description = "SMAB-T357:SMAB-T430 Transaction record verification for the imported Building Permit in XLS Format", dataProvider = "loginUsers", priority = 1, alwaysRun = true, enabled = false)
	...
	public void bldngPrmtsCreateManualEntry() throws Exception {
		salesforceStandardFunctions.login(users.BUSINESS_ADMIN);
		salesforceStandardFunctions.searchApps(apps.BUILDING_PERMITS);
