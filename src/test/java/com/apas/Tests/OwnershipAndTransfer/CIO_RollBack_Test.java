...
public class CIO_RollBack_Test extends TestBase implements testdata, modules, users {
	...
	String ownershipCreationData = testdata.OWNERSHIP_AND_TRANSFER_CREATION_DATA;
		...
		objCioTransfer.Click(objCioTransfer.getButtonWithText(objCioTransfer.nextButton));
		objCioTransfer.waitForElementToBeVisible(objCioTransfer.locateElement(objCioTransfer.transferSucessMessage, 10));
		
		String returnMessage = "";
		List<WebElement> SucessMessage=objCioTransfer.locateElements(objCioTransfer.transferSucessMessage, 30);
		
		for (int i=0; i<SucessMessage.size();i++)
			
			if(i==1)
			returnMessage=returnMessage+SucessMessage.get(i).getText();
		
			else
				returnMessage=returnMessage+" "+SucessMessage.get(i).getText();
		
		
		softAssert.assertTrue(returnMessage.trim().equals("Work Item has been returned by the approver."),
		...
		if(!objCioTransfer.waitForElementToBeVisible(20, objCioTransfer.newButton))
			driver.navigate().refresh();
		objCioTransfer.waitForElementToBeVisible(20, objCioTransfer.newButton);
		...
		driver.navigate().to("https://smcacre--" + System.getProperty("region").toLowerCase()
				+ ".lightning.force.com/lightning/r/Recorded_APN_Transfer__c/" + recordeAPNTransferID + "/view");
		objCioTransfer.waitForElementToBeVisible(objCioTransfer.quickActionButtonDropdownIcon,30);
