...
public class CIO_UnrecordedEvents_Test extends TestBase implements testdata, modules, users {
	 ...
	 */
	@Test(description = "SMAB-T4405 & SMAB-T4406: Verify APAS is able to do this transformation to store the mailing zip in the required format and determine the last four digits if only first five are provided. ", dataProvider = "loginSystemAdmin", dataProviderClass = DataProviders.class, groups = {
			"Regression", "ChangeInOwnershipManagement", "UnrecordedEvent" }, enabled = true)
	public void CIO_UnrecordedEvent_ZipCodeTransformationToNineDigits(String loginUser) throws Exception {
		...
		HashMap<String, ArrayList<String>> granteeHashMap  = objCIOTransferPage.getGridDataForRowString("1");
		String granteeForMailTo= granteeHashMap.get("Grantee/Retain Owner Name").get(0);
		ReportLogger.INFO("Grantee name: " + granteeForMailTo);
		 
		// Step 1: Login as CIO staff
		
		// STEP 2-7: Creating copy to mail to record
		driver.navigate().to("https://smcacre--" + execEnv
				+ ".lightning.force.com/lightning/r/Recorded_APN_Transfer__c/" + recordeAPNTransferID + "/view");
		objCIOTransferPage.waitUntilPageisReady(driver);
		...
		objParcelsPage.clickHyperlinkOnFieldValue(granteeForMailTo);
		objParcelsPage.waitUntilPageisReady(driver);*/
		...
		objParcelsPage.Click(objCIOTransferPage.mailingStatefield);
