...
//				WebElement adjacentCell = objBppTrend.locateElement("((//lightning-formatted-text[text() = 'Junk_A'] | //lightning-formatted-text[text() = 'Junk_B'])[1])//ancestor::td//preceding-sibling::td[1]", 20);
...
//	}
	...
	public void verify_BppTrend_CaaValImportRetryErrorRecordsAndApprove(String loginUser) throws Exception {
		...
		int fileCountBeforeRetrying = Integer.parseInt(objBppTrend.getFileCountFromHistoryTable());
		System.out.println("File count from history table: "+ fileCountBeforeRetrying);
		System.out.println("File count from Excel: "+ fileCountFromExcel);
		if(loginUser.contains("rpBusinessAdmin")) {
		...
		int importCountBeforeRetrying = Integer.parseInt(objBppTrend.getImportCountFromHistoryTable());
		System.out.println("Import count from history table: "+ importCountBeforeRetrying);
		System.out.println("Import count from Excel: "+ importCountFromExcel);
		if(loginUser.contains("rpBusinessAdmin")) {
		...
		int errorCountBeforeRetrying = Integer.parseInt(objBppTrend.getErrorCountFromHistoryTable());			
		System.out.println("Error count from history table: "+ errorCountBeforeRetrying);
		System.out.println("Error count from Excel: "+ errorCountFromExcel);
		if(loginUser.contains("rpBusinessAdmin")) {
		...
		for (int i = 0; i < allTables.size(); i++) {
			...
			for(int j = 1; j <= numberOfErrorRecordsUnderCurrentTable; j++) {
				//Step11: Editing the value of average column
				System.out.println("Locating junk cell");
				//objBppTrend.waitForElementToBeClickable(objBppTrend.tableCellWithJunkTextOnImportPageTale);
				//objBppTrend.Click(objBppTrend.tableCellWithJunkTextOnImportPageTale);
				objBppTrend.clickJunkDataCell(tableNumber);
				System.out.println("Clicked junk cell");
				Thread.sleep(5000);
				
				System.out.println("Locating edit icon in junk cell");
				objBppTrend.waitForElementToBeVisible(objBppTrend.editIconInImportPageTale, 10);
				objBppTrend.waitForElementToBeClickable(objBppTrend.editIconInImportPageTale, 10);
				objBppTrend.Click(objBppTrend.editIconInImportPageTale);
				System.out.println("Edit icon in junk cell clicked");
				
				//Step12: Entering a correct value in the average column text box
				valueToEnter = valueToEnter - 1;
				objBppTrend.enter(objBppTrend.inputBoxOnImportPage, Integer.toString(valueToEnter));
				
				System.out.println("Locating adjacent cell");
				objBppTrend.clickAdjacentCell(tableNumber);
				
				//Step13: Clicking on adjacent cell to move control from input box
				//System.out.println("Locating adjacent cell");
				//objBppTrend.waitForElementToBeClickable(objBppTrend.cellAdjacentToErrorDataCellOnImportPage, 10);			
				//WebElement adjacentCell = objBppTrend.locateElement("", 120);
				//System.out.println("Adjacent cell located");
				//objBppTrend.Click(objBppTrend.cellAdjacentToErrorDataCellOnImportPage);
				//System.out.println("Adjacent cell clicked");
				Thread.sleep(1000);
			}			
		}
