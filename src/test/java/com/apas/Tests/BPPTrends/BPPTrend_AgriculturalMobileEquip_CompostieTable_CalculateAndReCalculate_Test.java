...
public class BPPTrend_AgriculturalMobileEquip_CompostieTable_CalculateAndReCalculate_Test extends TestBase {
	 ...
	 */
	@Test(description = "SMAB-T200,SMAB-T268,SMAB-T190,SMAB-T194,SMAB-T198: Performing validation on AGRICULTURAL MOBILE EQUIP COMPOSITE FACTORS before and after calculation", groups = {"smoke","regression","BPPTrend"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class)
	public void BppTrend_AgriculturalMobileCompositeFactors_CalculateAndCompare(String loginUser) throws Exception {
		...
		objBppTrnPg.updateMinimumEquipmentIndexFactorValue("Agricultural", CONFIG.getProperty("minEqipFactorAgriculturalDefaultValue"), rollYear);
		...
		System.out.println("Clicking on table name");
		objBppTrnPg.clickOnTableOnBppTrendPage(tableName);
		System.out.println("Clicked on table name");
		...
		String actPopUpMsg = objBppTrnPg.waitForErrorPopUpMsgOnCalculateClick(30);
		String expPopUpMsg = CONFIG.getProperty("popUpMsgPostCalculation");
		softAssert.assertContains(actPopUpMsg, expPopUpMsg, "SMAB-T190: Validating pop up message on calculating the table data");
				...
				if (acquiredYearDataFromUI.size() == acquiredYearDataFromExcel.size()) {
					for (int j = 0; j < acquiredYearDataFromExcel.size(); j++) {
						if (!(acquiredYearDataFromExcel.get(j).equals(acquiredYearDataFromUI.get(j)))) {
							isDataMatched = false;
							softAssert.assertTrue(false, "SMAB-T200: Data for '"+ tableName +" 'for year acquired '" + currentKey + "' and column named '"+ columnNames.get(j) + "' MACTHED. Excel data: "+ acquiredYearDataFromExcel.get(j) + " || UI Data: " + acquiredYearDataFromUI.get(j), true);
						}
					}
					if(isDataMatched) {
						softAssert.assertTrue(true, "SMAB-T200: UI Data for '"+ tableName +" 'for year acquired '"+ currentKey +"' MATCHED with Trend Calculator data", true);
					}
				} else {
					//System.setProperty("isElementHighlightedDueToFailre", "true");
					softAssert.assertTrue(false, "SMAB-T200: Count of columns for '"+ tableName +" 'for year acquired '"+ currentKey +"' MACTHED", true);
				}
			} else {
				//System.setProperty("isElementHighlightedDueToFailre", "true");
				softAssert.assertTrue(false, "SMAB-T200: Data for '"+ tableName +" 'for year acquired '"+ currentKey +"' is PRESENT in UI table", true);
			}
		}
		if(!isDataMatched) {
	 ...
	 */
	@Test(description = "SMAB-T195,SMAB-T196: Performing validation on AGRICULTURAL MOBILE EQUIP COMPOSITE FACTORS before and after calculation", groups = {"regression","BPPTrend"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class)
	public void BppTrend_AgriculturalMobileCompositeFactors_ReCalculateAndCompare(String loginUser) throws Exception {
		...
		objBppTrnPg.updateMinimumEquipmentIndexFactorValue("Agricultural", CONFIG.getProperty("minEqipFactorUpdatedValue"), rollYear);
		...
		String actPopUpMsg = objBppTrnPg.waitForErrorPopUpMsgOnCalculateClick(30);
		String expPopUpMsg = CONFIG.getProperty("popUpMsgPostCalculation");
		softAssert.assertContains(actPopUpMsg, expPopUpMsg, "SMAB-T195: Validating pop up message on calculating the table data");
		...
		objBppTrnPg.updateTrendSettingInExcel("Agricultural", CONFIG.getProperty("minEqipFactorUpdatedValue"));
		Map<String, List<Object>> dataMapFromExcel = objBppTrnPg.retrieveDataFromExcelForGivenTable(fileName, tableName);
