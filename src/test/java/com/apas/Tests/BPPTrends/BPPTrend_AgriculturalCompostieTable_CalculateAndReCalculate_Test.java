...
public class BPPTrend_AgriculturalCompostieTable_CalculateAndReCalculate_Test extends TestBase {
	 ...
	 */
	@Test(description = "SMAB-T209,SMAB-T269,SMAB-T190,SMAB-T194,SMAB-T198: Performing validation on Agricultural COMPOSITE FACTORS before and after calculation", groups = {"smoke","regression","BPPTrend"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class)
	public void verify_BppTrend_AgriculturalCompositeFactors_CalculateAndCompare(String loginUser) throws Exception {
		...
		objSoftAssert.assertTrue(isReCalculateBtnDisplayed, "ReCalculate button is visible for calculated table");
		...
		isCalculateAllBtnDisplayed = objBppTrnPg.isCalculateAllBtnVisible(10);
				...
				if (acquiredYearDataFromUI.size() == acquiredYearDataFromExcel.size()) {
					for (int j = 0; j < acquiredYearDataFromExcel.size(); j++) {
						if (!(acquiredYearDataFromExcel.get(j).equals(acquiredYearDataFromUI.get(j)))) {
							isDataMatched = false;
							softAssert.assertTrue(false, "SMAB-T209: Data for '"+ tableName +" 'for year acquired '" + currentKey + "' and column named '"+ columnNames.get(j) + "' MACTHED. Excel data: "+ acquiredYearDataFromExcel.get(j) + " || UI Data: " + acquiredYearDataFromUI.get(j), true);
						}
					}
					if(isDataMatched) {
					...
					softAssert.assertTrue(false, "SMAB-T209: Count of columns for '"+ tableName +" 'for year acquired '"+ currentKey +"' MACTHED", true);
				}
			} else {
				//System.setProperty("isElementHighlightedDueToFailre", "true");
	 ...
	 */
	@Test(description = "SMAB-T195,SMAB-T196: Performing validation on AGRICULTURAL COMPOSITE FACTORS before and after calculation", groups = {"smoke","regression","BPPTrend"}, dataProvider = "loginBusinessAdmin", dataProviderClass = DataProviders.class)
	public void verify_BppTrend_AgriculturalCompositeFactors_ReCalculateAndCompare(String loginUser) throws Exception {
		...
		String actPopUpMsg = objBppTrnPg.waitForSuccessPopUpMsgOnCalculateClick(60);
		String expPopUpMsg = CONFIG.getProperty("popUpMsgPostCalculation");
