...
public class Parcel_Management_CombineMappingAction_Test extends TestBase implements testdata, modules, users {
	...
	public void ParcelManagement_VerifyParcelGenerationForCombineMappingAction(String loginUser) throws Exception {
		...
		//Getting parcels that are Active and with Ownership record
		...
		//Data Manipulation to test APN Generation scenarios
		String concatenateAPNWithSameOwnership = apn1+","+apn2;
		...
		//Create the next available APN for the APN fetched above
		...
		//Query last APN in Map Book and Map Page for Condo parcel
		...
		// Step 9: Click previous button and update parent parcel field with Condo as first parent parcel
		ReportLogger.INFO("Click PREVIOUS button");
		objMappingPage.Click(objMappingPage.getButtonWithText(objMappingPage.previousButton));
		objMappingPage.waitForElementToBeVisible(6, objMappingPage.reasonCodeField);
		...
		String queryLastAPNInMapBookAndPage = "SELECT Name, Id from parcel__c where Id NOT in (Select parcel__c FROM Property_Ownership__c)and name like '%990' and Status__c = 'Active' limit 1";
	...
	public void AParcelManagement_VerifyParcelCombineMappingAction(String loginUser) throws Exception {
       ...
       /* String queryParcelSize = "SELECT Lot_Size_SQFT__c FROM Parcel__c  where Name = '" + apn1 + "'";
		...
		softAssert.assertEquals(gridDataHashMap.get("District/Neighborhood").get(0),responseNeighborhoodDetails.get("Name").get(0),
				"SMAB-T2356: Validation that System populates District/Neighborhood from the parent parcel");
