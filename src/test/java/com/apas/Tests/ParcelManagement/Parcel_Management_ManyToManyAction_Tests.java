...
public class Parcel_Management_ManyToManyAction_Tests extends TestBase implements testdata, modules, users {
	...
	@Test(description = "SMAB-T3805,SMAB-T3806,SMAB-T3777,SMAB-T3787,SMAB-T3788,SMAB-T3854,SMAB-T3888,SMAB-T3855,SMAB-T3889,SMAB-T3813:Verify that User is able to perform a \"Many to Many\" mapping action for a Parcel   from a work item and verify output with update chars and verify PUC", dataProvider = "loginMappingUser", dataProviderClass = DataProviders.class, groups = {
			"Regression", "ParcelManagement" }, enabled = true)
	public void ParcelManagement_Verify_ManyToManyOutput1(String loginUser)
			throws Exception{
		...
		Thread.sleep(5000);
		   ...
		   // Completing the work Item	
			...
			objParcelsPage.waitForElementToBeVisible(trail.businessEventCharacteristicsAuditTrail, 10);
			softAssert.assertTrue(objMappingPage.verifyElementVisible(trail.businessEventCharacteristicsAuditTrail),
			...
			objParcelsPage.Click(trail.businessEventCharacteristicsAuditTrail);
			objParcelsPage.waitForElementToBeVisible(trail.relatedBusinessRecords);
			objParcelsPage.Click(trail.relatedBusinessRecords);
			objWorkItemHomePage.waitForElementToBeVisible(20, trail.linkedRecord);
			String trailSubject = trail.linkedRecord.getText();
